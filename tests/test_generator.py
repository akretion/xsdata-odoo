...
class OdooGeneratorTests(FactoryTestCase):
    ...
    def test_render_cfdi(self):
        os.environ["XSDATA_SCHEMA"] = "l10n_mx_cfdi"
        os.environ["XSDATA_VERSION"] = "4_0"
        os.environ['XSDATA_LANG'] = "spanish"

        runner = CliRunner()
        current_dir = Path(__file__).parent
        schema = current_dir / 'fixtures/cfdi/4/cfdv40.xsd'
        os.chdir(current_dir)

        result = runner.invoke(
            cli, [str(schema), "--output", "odoo"],
        ...
        # read generated file
        generated_file_path = current_dir / 'generated/cfdv40.py'
        generated_file_content = ''
        with open(generated_file_path) as f:
            generated_file_content = f.read()

        # extract lines containing '_name ='
        lines = [line.strip() for line in generated_file_content.splitlines() if '_name =' in line]
        ...
        self.assertFalse(duplicates, 'There are duplicate _name values:\n%s' % "\n".join(duplicates))
